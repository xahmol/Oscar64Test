 ...
 * @param nCurBitOffset write index into output buffer, of current byte being filled with bits
 * @param nBitValue value to write
 */
static int lzsa_write_bit_v2(unsigned char *pOutData, int nOutOffset, const int nMaxOutDataSize, int *nCurBitOffset, int nBitValue) {
   if (nOutOffset < 0) return -1;

   if ((*nCurBitOffset) == -1) {
      if (nOutOffset >= nMaxOutDataSize) return -1;
      (*nCurBitOffset) = nOutOffset;
      pOutData[nOutOffset++] = 0x01;
   }
   if ((pOutData[*nCurBitOffset] & 0x80) == 0x80) {
      pOutData[*nCurBitOffset] = (pOutData[*nCurBitOffset] << 1) | (nBitValue != 0);
      (*nCurBitOffset) = -1;
   } else {
      pOutData[*nCurBitOffset] = (pOutData[*nCurBitOffset] << 1) | (nBitValue != 0);
   }
...
static inline int lzsa_write_literals_varlen_v2(unsigned char *pOutData, int nOutOffset, const int nMaxOutDataSize, int *nCurNibbleOffset, int nLength) {
   int i;
   int s = bit_size(nLength) * 2 + 1 + 1;
   for (i = 0; i < s; i++) nOutOffset = lzsa_write_bit_v2(pOutData, nOutOffset, nMaxOutDataSize, nCurNibbleOffset, nLength - LITERALS_RUN_LEN_V2);
   return nOutOffset;
   if (nLength >= LITERALS_RUN_LEN_V2) {
      if (nLength < (LITERALS_RUN_LEN_V2 + 15)) {
...
static inline int lzsa_write_match_offset_v2(unsigned char *pOutData, int nOutOffset, const int nMaxOutDataSize, int *nCurNibbleOffset, int nLength) {
   int i;
   int s = bit_size(nLength >> 7) * 2 + 1;
   for (i = 0; i < s; i++) nOutOffset = lzsa_write_bit_v2(pOutData, nOutOffset, nMaxOutDataSize, nCurNibbleOffset, nLength - LITERALS_RUN_LEN_V2);
   for (i = 0; i < 7; i++) {
         nOutOffset = lzsa_write_bit_v2(pOutData, nOutOffset, nMaxOutDataSize, nCurNibbleOffset, nLength - MATCH_RUN_LEN_V2);
   }
   return nOutOffset;
}
...
static void lzsa_optimize_forward_v2(lzsa_compressor *pCompressor, const unsigned char *pInWindow, lzsa_match *pBestMatch, const int nStartOffset, const int nEndOffset, const int nReduce, const int nInsertForwardReps, const int nArrivalsPerPosition) {
   ...
   for (i = nStartOffset; i != nEndOffset; i++) {
      ...
      for (m = 0; m < NMATCHES_PER_INDEX_V2 && match[m].length; m++) {
         ...
         int nNoRepmatchOffsetCost = bit_size(nMatchOffset >> 7) * 2 + 1 + 7;//(nMatchOffset <= 32) ? 4 : ((nMatchOffset <= 512) ? 8 : ((nMatchOffset <= (8192 + 512)) ? 12 : 16));
      ...
      if (pMatch->length >= MIN_MATCH_SIZE_V2) {
         if ((i + pMatch->length) < nEndOffset /* Don't consider the last match in the block, we can only reduce a match inbetween other tokens */) {
            ...
            if (nNextIndex < nEndOffset && pBestMatch[nNextIndex].length >= MIN_MATCH_SIZE_V2) {
               ...
               if (nRepMatchOffset && pMatch->offset != nRepMatchOffset && (pBestMatch[nNextIndex].offset != pMatch->offset || pBestMatch[nNextIndex].offset == nRepMatchOffset ||
                  ((bit_size(pMatch->offset) >> 7) * 2 + 1 + 7) > ((bit_size(pBestMatch[nNextIndex].offset) >> 7) * 2 + 1 + 7))) {
               ...
               if (pBestMatch[nNextIndex].offset && pMatch->offset != pBestMatch[nNextIndex].offset && nRepMatchOffset != pBestMatch[nNextIndex].offset) {
                  ...
                  if (i > pBestMatch[nNextIndex].offset && (i - pBestMatch[nNextIndex].offset + pMatch->length) <= nEndOffset) {
                     ...
                     if (nMaxLen >= pMatch->length) {
                        ...
                        nPartialSizeBefore += bit_size(pMatch->offset >> 7) * 2 + 1 + 7; //(pMatch->offset <= 32) ? 4 : ((pMatch->offset <= 512) ? 8 : ((pMatch->offset <= (8192 + 512)) ? 12 : 16));
               ...
               if (pMatch->length < 9 /* Don't waste time considering large matches, they will always win over literals */) {
                  ...
                  if (pMatch->offset != nRepMatchOffset)
                     nCurCommandSize += bit_size(pMatch->offset >> 7) * 2 + 1 + 7;//(pMatch->offset <= 32) ? 4 : ((pMatch->offset <= 512) ? 8 : ((pMatch->offset <= (8192 + 512)) ? 12 : 16));
                  ...
                  if (pBestMatch[nNextIndex].offset != pMatch->offset)
                     nNextCommandSize += bit_size(pBestMatch[nNextIndex].offset >> 7) * 2 + 1 + 7;//(pBestMatch[nNextIndex].offset <= 32) ? 4 : ((pBestMatch[nNextIndex].offset <= 512) ? 8 : ((pBestMatch[nNextIndex].offset <= (8192 + 512)) ? 12 : 16));
                  ...
                  if (pBestMatch[nNextIndex].offset != nRepMatchOffset)
                     nReducedCommandSize += bit_size(pBestMatch[nNextIndex].offset >> 7) * 2 + 1 + 7;//(pBestMatch[nNextIndex].offset <= 32) ? 4 : ((pBestMatch[nNextIndex].offset <= 512) ? 8 : ((pBestMatch[nNextIndex].offset <= (8192 + 512)) ? 12 : 16));
                  ...
                  if (nRepMatchOffset && nRepMatchOffset != nPrevRepMatchOffset && nRepMatchLen >= MIN_MATCH_SIZE_V2 && nRepMatchOffset != pBestMatch[nNextIndex].offset && nRepIndex > pBestMatch[nNextIndex].offset &&
                     ...
                     nReplaceRepOffset = 1;
                     nReducedCommandSize -= bit_size(nRepMatchOffset >> 7) * 2 + 1 + 7;//(nRepMatchOffset <= 32) ? 4 : ((nRepMatchOffset <= 512) ? 8 : ((nRepMatchOffset <= (8192 + 512)) ? 12 : 16));
                  }
         ...
         if ((i + pMatch->length) <= nEndOffset && pMatch->offset > 0 && pMatch->length >= MIN_MATCH_SIZE_V2 &&
            ...
            if (pBestMatch[i + pMatch->length].offset != pMatch->offset)
               nCurPartialSize += bit_size(pBestMatch[i + pMatch->length].offset >> 7) * 2 + 1 + 7;//(pBestMatch[i + pMatch->length].offset <= 32) ? 4 : ((pBestMatch[i + pMatch->length].offset <= 512) ? 8 : ((pBestMatch[i + pMatch->length].offset <= (8192 + 512)) ? 12 : 16));

            if (nNextOffset != pBestMatch[i + pMatch->length].offset)
               nCurPartialSize += bit_size(nNextOffset >> 7) * 2 + 1 + 7;//(nNextOffset <= 32) ? 4 : ((nNextOffset <= 512) ? 8 : ((nNextOffset <= (8192 + 512)) ? 12 : 16));
            ...
            if (nNextOffset != pMatch->offset)
               nReducedPartialSize += bit_size(nNextOffset >> 7) * 2 + 1 + 7;//(nNextOffset <= 32) ? 4 : ((nNextOffset <= 512) ? 8 : ((nNextOffset <= (8192 + 512)) ? 12 : 16));
...
static int lzsa_write_block_v2(lzsa_compressor *pCompressor, lzsa_match *pBestMatch, const unsigned char *pInWindow, const int nStartOffset, const int nEndOffset, unsigned char *pOutData, const int nMaxOutDataSize) {
   ...
   for (i = nStartOffset; i < nEndOffset; ) {
      ...
      //XXX TODO check here also for overrun of match for inline depacking
      if (pMatch->length >= MIN_MATCH_SIZE_V2) {
         int nMatchOffset = pMatch->offset;
         int nMatchLen = pMatch->length;
         int nEncodedMatchLen = nMatchLen - MIN_MATCH_SIZE_V2;
         ...
         if (nMatchOffset == nRepMatchOffset) {
            ...
            /* 5 bits */
            if (nMatchOffset <= 32) {
               nTokenOffsetMode = 0x00 | ((((-nMatchOffset) & 0x01) << 6));
               nOffsetSize = 4;
            }
            /* 9 bits XXX TODO could use add 0x20 to make use of full offset range? */
            else if (nMatchOffset <= 512) {
               nTokenOffsetMode = 0x80 | ((((-nMatchOffset) & 0x100) >> 3));
               nOffsetSize = 8;
            }
            /* 13 bits */
            else if (nMatchOffset <= (8192 + 512)) {
               nTokenOffsetMode = 0x20 | ((((-(nMatchOffset - 512)) & 0x0100) >> 2));
               nOffsetSize = 12;
            }
            /* 16 bits */
            else {
         ...
         //pOutData[nOutOffset++] = nTokenOffsetMode | (nTokenLiteralsLen << LITERALS_LEN_POS_V2) | (nTokenMatchLen << MATCH_LEN_POS_V2);
         ...
         if ((nTokenOffsetMode & 0x80) == 0x00) {
            if ((nTokenOffsetMode & 0x20) == 0x00) {
            ...
            if ((nTokenOffsetMode & 0xc0) == 0x80) {
               //pOutData[nOutOffset++] = (-nMatchOffset) & 0xff;
            }
            else if ((nTokenOffsetMode & 0xe0) == 0xc0) {
         ...
         if (nMatchOffset != nRepMatchOffset)
             nOutOffset = lzsa_write_match_offset_v2(pOutData, nOutOffset, nMaxOutDataSize, &nCurNibbleOffset, nMatchOffset);
...

   {
      ...
      if (pCompressor->flags & LZSA_FLAG_RAW_BLOCK) {
         //pOutData[nOutOffset++] = (nTokenLiteralsLen << LITERALS_LEN_POS_V2) | 0xe0 | (MATCH_RUN_LEN_V2 << MATCH_LEN_POS_V2);
      } else {
	printf ("nOut first: %x\n", nOutOffset);
         if (nCurNibbleOffset != -1) {
         ...
         pCompressor->end_position = nOutOffset;
         //pOutData[nOutOffset++] = (nTokenLiteralsLen << LITERALS_LEN_POS_V2) | 0x00;
	 //endposition!!!! XXX TODO
         //nEndPosition = nOutOffset;
         //pCompressor->end_position = 0;//nOutOffset;
